...
import { comparePassword } from "../helper/hashHelpers.js";
import { generate2faCode, generateToken } from "../helper/authHelpers.js";
import {
...
const forgetPassword = async (req, res, next) => {
  ...
  try {
    ...
    const token = generateTokens({
      ...
      signature: process.env.ACCESS_TOKEN_SECRET || "ACCESS_TOKEN_SECRET",
      expiresIn: "20m",
    });
...
const resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    user.password = await passwordToHash(encryptPassword(password));
